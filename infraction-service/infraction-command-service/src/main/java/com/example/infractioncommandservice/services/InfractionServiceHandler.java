...
import commonapi.NewInfractionCommand;
import commonapi.VehicleCreatedEvent;
import commonapi.VehicleOverSpeedDetectedEvent;
...
public class InfractionServiceHandler {
    ...
    public void on(VehicleOverSpeedDetectedEvent event){
        ...
        if(event.getInfractionId()==null) return;
        List<? extends DomainEventMessage<?>> domainEventMessages=new ArrayList<>();
        DomainEventStream domainEventStream = eventStore.readEvents(event.getInfractionId());
